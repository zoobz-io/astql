...
Adds a wait policy to the row lock. Chain after a lock mode:

```go
astql.Select(t).ForUpdate().SkipLocked() // FOR UPDATE SKIP LOCKED
astql.Select(t).ForUpdate().NoWait()     // FOR UPDATE NOWAIT
```

`SkipLocked` skips rows already locked by another transaction (the canonical
work-queue / claim pattern); `NoWait` errors immediately instead of blocking.
Calling either without a lock mode is an error. PostgreSQL only — other
dialects return `UnsupportedFeatureError`.
...
- Row-lock wait policy (`SKIP LOCKED` / `NOWAIT`) — PostgreSQL only
