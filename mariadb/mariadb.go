...
func (r *Renderer) RenderCompound(query *types.CompoundQuery) (*types.QueryResult, error) {
	...
	// MySQL supports parentheses around SELECT in compound queries
	sql.WriteString("(")
...
func (r *Renderer) renderInsert(ast *types.AST, sql *strings.Builder, addParam func(types.Param) string) error {
	...
	// ON CONFLICT → ON DUPLICATE KEY UPDATE for MySQL
	if ast.OnConflict != nil {
		switch ast.OnConflict.Action {
		case types.DoNothing:
			// MySQL: INSERT IGNORE or ON DUPLICATE KEY UPDATE with no-op
			// Using ON DUPLICATE KEY UPDATE with a dummy assignment
			if len(fieldObjs) > 0 {
				firstField := r.quoteIdentifier(fieldObjs[0].Name)
				sql.WriteString(" ON DUPLICATE KEY UPDATE ")
				sql.WriteString(firstField)
				sql.WriteString(" = ")
				sql.WriteString(firstField)
			}
		case types.DoUpdate:
...
// renderPaginationValue renders a LIMIT or OFFSET value.
...
func (r *Renderer) mapCastType(castType types.CastType) string {
	...
	case types.CastBoolean:
		return "UNSIGNED" // MySQL uses 0/1 for boolean
...
func (r *Renderer) renderStringExpression(expr types.StringExpression, ctx *renderContext) (string, error) {
	...
	case types.StringLength:
		// MySQL uses LENGTH() or CHAR_LENGTH()
		sql.WriteString("LENGTH(")
		sql.WriteString(r.renderField(expr.Field))
		sql.WriteString(")")
	case types.StringSubstring:
		// MySQL uses SUBSTRING(string, start, length)
...
func (r *Renderer) renderDateExpression(expr types.DateExpression, _ *renderContext) (string, error) {
	...
	case types.DateCurrentDate:
		sql.WriteString("CURDATE()")
	case types.DateCurrentTime:
		sql.WriteString("CURTIME()")
	...
	case types.DateTrunc:
		...
		// MySQL doesn't have DATE_TRUNC, need to use DATE_FORMAT or similar
		// For simplicity, use DATE() for day truncation, otherwise error
		switch expr.Part {
		case types.PartDay:
		...
		case types.PartMonth:
			sql.WriteString("DATE_FORMAT(")
			sql.WriteString(r.renderField(*expr.Field))
			sql.WriteString(", '%Y-%m-01')")
		case types.PartYear:
			sql.WriteString("DATE_FORMAT(")
			sql.WriteString(r.renderField(*expr.Field))
